...
| Statement | Cross-check result |
|---|---|
| Checking crossover against *any* of the 13 marked levels (not just the entry strike's own CE/PE High/Low band) | **RESOLVED, CONFIRMED (2026-08-01).** Pointed to a real, already-supplied example — 30-July's 24250-TOP Row 1 vs Row 2: Row 1's entry level (120.1) and competitor level (121.5) both belong to 24250 itself; Row 2's entry level (98.3) belongs to a *different* strike (24200), while its competitor level (121.5) stays on 24250. Product Owner confirmed directly: **"yes, correct, use the wider ladder."** `WinnerEngine`/`QualificationEngine` must check against the full marked-level ladder, not a single strike's own band — this is no longer an open question. |

**Workflow clarification (Product Owner, 2026-08-01), verbatim:** "dont worry about the strikes, the logic is on top/bottom. First maintain the data like below: Get the top/bottom strike CE/PE +- first 5mins high and low, then assign based on top and bottom strike, then watch both top and bottom strike carefully for entry and exit SL, TSL."

**Reading (not the evidence of record):** the four premium streams actually watched for crossing are **only Top's own CE, Top's own PE, Bottom's own CE, and Bottom's own PE** — not all 13 strikes' own premiums. The wider 13-level ladder supplies the *marked reference levels* those four streams are checked against (which strike's High/Low a crossing lands on can vary, as Row 2 above shows), but the premiums being watched are always Top's or Bottom's own CE/PE, never a third strike's own premium. This is now a complete, implementable specification: **which strikes to watch (Top, Bottom) is fixed and small; which levels they can cross (the wider ladder) is wide and can span strikes.**
...
| Stop Loss = S-1 (CE) / S+1 (PE) | **CONFIRMED for both sides** (Session 3). Rule 2 distinguishes Support (`CE(S-1)`/`PE(S+1)`, same side) from Competitor Exit (`PE(S-1)`/`CE(S+1)`, opposite side) — SL = Support exactly, on both sides: Product Owner confirmed `CE(S-1)` directly, then confirmed the PE side is symmetric ("PE side same as CE, S+1"). No longer an inference — see `stop_loss_session3_intake_2026-07-31.md`. |
...
**Counter Example for this day:** Trade 3 (CE, 24000 anchor) — did **not** exit via Target or SL; exited via "CE Resistance hit first" at 101.6, between its SL (78.2) and Target (114). A real instance where the standard Target/Competitor/SL pattern did not simply hold, satisfying `evidence_acceptance_checklist.md`'s Counter Example requirement using data already supplied, retroactively labeled rather than newly requested.
