"""Shared fixtures for the Domain layer test suite.

Provides only generic, structurally-valid values (UUIDs, timestamps,
decimals) - no trading-meaningful data, consistent with the Domain
layer itself containing no trading logic.
"""

from __future__ import annotations

import uuid
from datetime import date, datetime
from decimal import Decimal

import pytest


@pytest.fixture
def valid_uuid() -> uuid.UUID:
    return uuid.uuid4()


@pytest.fixture
def another_uuid() -> uuid.UUID:
    return uuid.uuid4()


@pytest.fixture
def valid_datetime() -> datetime:
    return datetime(2026, 7, 3, 9, 20, 0)  # noqa: DTZ001


@pytest.fixture
def valid_date() -> date:
    return date(2026, 7, 3)


@pytest.fixture
def valid_decimal() -> Decimal:
    return Decimal(24050)
